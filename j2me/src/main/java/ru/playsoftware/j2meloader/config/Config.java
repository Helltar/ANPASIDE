...
public class Config {
	...
	static {
		...
		String appName = "ANPASIDE";
		SCREENSHOTS_DIR = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES)
				+ "/" + appName;
		SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(context);
		// keep the emulator's converted apps, profiles and record stores out of the
		// ide working directory, which holds the user's pascal projects
		String path = context.getExternalFilesDir(null).getPath() + "/emulator";
		initDirs(path);
